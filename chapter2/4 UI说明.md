...
|  字段名  |     字段     | 值                                                           |
| :------: | :----------: | :----------------------------------------------------------- |
| 文本内容 | Text_Content | {" { “游戏速度 正常”, "游戏速度 加速",  "游戏速度 超快 "} ",<br />" { "音量 0", "音量 1", "音量 2", "音量 3" } ",<br />" { "音效 0", "音效 1", "音效 2", "音效 3" } ", <br />"返回主界面",<br />"取消"} |
| 文本颜色 |  Text_Color  | {“#FFFFFF”, <br />“#FFFFFF”, <br />“#FFFFFF”, <br />“#FF0000”, <br />"#B5B5B5"} |
...
![image-20210915141127671](https://i.loli.net/2021/09/15/ZRWcA9evVoC6LiY.png)

（1）点击战斗开始键后，画面由准备阶段进入战斗阶段，该button中Start_icon将会改变为Pause_Icon（此处无法通过button组件中的sprite swap实现，需要程序进行显示和隐藏变换），变更功能为暂停战斗，当再次点击该按键后，战斗画面暂停，Pause_Icon图标将重新变换回Start_Icon，以此类推

（2）如果在程序无法实现在战斗中实时改变速度，那么在点击设置按键时，也应将战斗画面暂停
